...
class WhisperEngine:
    ...
    def transcribe_audio(self, audio_path: Path, progress_callback=None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self.device == "cuda":
                self._log_gpu_memory_usage("before transcription")
    ...
    def estimate_processing_time(self, duration_seconds: float) -> str:
        ...
        if self.device == "cuda":
            # RTX 4090 is very fast for Whisper
            ratio = 0.05  # GPU is ~20x faster than CPU for Whisper
        elif self.device == "mps":
            ratio = 0.15  # Apple Silicon is ~7x faster
        else:
